...
export interface Interface {
  ...
  readonly readLogs: (directory: string, input: ReadLogsInput) => Effect.Effect<{ lines: Live.DevLogLine[]; cursor: number }>
}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    function decide(
      ...
      match: Match,
      now: number,
      item: { event?: Live.Telemetry; backendLine?: Live.DevLogLine; ts: number; correlatedLogs: ReadonlyArray<Live.DevLogLine> },
    ): Admission | undefined {
    ...
    // (same-signature) and DIGEST_WINDOW_MAX (wakes per window) remain the throttles.
    function admitBatch(channel: Channel, admissions: ReadonlyArray<Admission>, now: number): LiveDigest.Digest | undefined {
      ...
      const budgeted = now - channel.lastUserActivity >= USER_QUIET_MS && inWindow.length < DIGEST_WINDOW_MAX
      if (!budgeted) {
      ...
      const recentActions = channel.entries
        .filter(
          (entry) =>
            entry.event.kind === "navigation" || entry.event.kind === "click" || entry.event.kind === "input",
        )
    ...
    return Service.of({
      ...
      bind: Effect.fn("LiveGateway.bind")(function* (directory, sessionID) {
        const channel = channelFor(directory)
        // Armed waits belong to the bound session; rebinding to a different one discards them.
        if (channel.sessionID !== sessionID) yield* clearWaits(channel)
      ...
      readLogs: Effect.fn("LiveGateway.readLogs")(function* (directory, input) {
        ...
        const retainedFrom =
          input.since === undefined
            ? 0
            : Math.max(0, channel.logs.length - Math.max(0, channel.logSeq - input.since))
        const limit = input.limit ?? 100
